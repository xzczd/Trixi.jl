...
@muladd begin
  ...
  """
  struct CompressibleEulerMultichemistryEquations2D{NVARS, NCOMP, RealT<:Real} <: AbstractCompressibleEulerMultichemistryEquations{2, NVARS, NCOMP}
  ...
  function initial_condition_knallgas_detonation(x, t, equations::CompressibleEulerMultichemistryEquations2D)
  
    if x[1] <= 0.5    # ready to burn
      u     = 10.0
      v     = 0.0
      p     = 40.0  # T = 10
      H2    = 0.0
      ...
      H2O   = 0.63 * 2.0 
      N2    = 0.2  * 2.0
    elseif x[1] > 0.5 && x[2] >= 1.2   # why is it so silent here?
      u     = 0.0
      v     = 0.0
      p     = 1.0   # T = 1
      H2    = 0.0
      ...
      H2O   = 0.63
      N2    = 0.2
    elseif x[1] > 0.5 && x[2] < 1.2  # everything burnt
      u     = 0.0
      v     = 0.0
      p     = 1.0   # T = 1
      H2    = 0.08
  ...
  function boundary_condition_knallgas_detonation(u_inner, orientation, direction, x, t,
    ...
    u_boundary = initial_condition_knallgas_detonation(x, t, equations)
  ...
  @inline function chemistry_knallgas_detonation(u, dt, equations::CompressibleEulerMultichemistryEquations2D)
    # Same settings as in `initial_condition`
    rho_v1, rho_v2, rho_e, H2, O2, OH, H2O, N2  = u
  ...
  """
      initial_condition_shock_bubble(x, t, equations::CompressibleEulerMultichemistryEquations2D{6, 3})
  Adaption of the shock-bubble testcase for multichemistry Euler equations to 3 components
  - Ayoub Gouasmi, Karthik Duraisamy, Scott Murman
    Formulation of Entropy-Stable schemes for the multichemistry compressible Euler equations
    [arXiv: 1904.00972](https://arxiv.org/abs/1904.00972)
  """
  function initial_condition_shock_bubble(x, t, equations::CompressibleEulerMultichemistryEquations2D{6, 3})
    # bubble test case, see Gouasmi et al. https://arxiv.org/pdf/1904.00972
    # other reference: https://www.researchgate.net/profile/Pep_Mulet/publication/222675930_A_flux-split_algorithm_applied_to_conservative_models_for_multicomponent_compressible_flows/links/568da54508aeaa1481ae7af0.pdf
  ...
  """
      initial_condition_shock_bubble(x, t, equations::CompressibleEulerMultichemistryEquations2D{10, 7})
  Adaption of the shock-bubble testcase for multichemistry Euler equations to 7 components
  - Ayoub Gouasmi, Karthik Duraisamy, Scott Murman
    Formulation of Entropy-Stable schemes for the multichemistry compressible Euler equations
    [arXiv: 1904.00972](https://arxiv.org/abs/1904.00972)
  """
  function initial_condition_shock_bubble(x, t, equations::CompressibleEulerMultichemistryEquations2D{10, 7})
    # bubble test case, see Gouasmi et al. https://arxiv.org/pdf/1904.00972
    # other reference: https://www.researchgate.net/profile/Pep_Mulet/publication/222675930_A_flux-split_algorithm_applied_to_conservative_models_for_multicomponent_compressible_flows/links/568da54508aeaa1481ae7af0.pdf
  ...
  function flux_hllc(u_ll, u_rr, orientation::Integer, equations::CompressibleEulerMultichemistryEquations2D)
    ...
    p_ll = (gamma - 1) * (rho_e_ll - 1/2 * rho_ll * (v1_ll^2 + v2_ll^2) - p_chem_ll)
    c_ll = sqrt(gamma*p_ll/rho_ll)
  
    v1_rr = rho_v1_rr / rho_rr
    v2_rr = rho_v2_rr / rho_rr
    e_rr  = rho_e_rr / rho_rr
    p_rr = (gamma - 1) * (rho_e_rr - 1/2 * rho_rr * (v1_rr^2 + v2_rr^2) - p_chem_rr)
    ...
    ekin_roe = 0.5 * (vel_roe^2 + ekin_roe / sum_sqrt_rho^2) + 0.5 * (p_chem_ll + p_chem_rr)
    H_ll = (rho_e_ll + p_ll) / rho_ll
    H_rr = (rho_e_rr + p_rr) / rho_rr # 48.5
    #println("H_ll: ",H_ll)
    #println("H_rr: ",H_rr)
    #println("sqrt_rho_ll: ",sqrt_rho_ll)
    #println("sqrt_rho_rr: ",sqrt_rho_rr)
    #println("sum_sqrt_rho: ",sum_sqrt_rho)
    ...
    return vcat(f_other, f_rho) #SVector(f1, f2, f3, f4)
  end
